...
def compute_fbeta(
    precision: float, recall: float, beta: float = 2.0, eps: float = 1e-13
) -> float:
    beta_sq = beta**2
...
class SpanF1:
    ...
    def __call__(self, batched_predicted_spans, batched_gold_spans, sentences=None):
        ...
        for predicted_spans, gold_spans in zip(
            batched_predicted_spans, batched_gold_spans
        ):
            gold_spans_set = set(
                [x for x, y in gold_spans.items() if y not in non_entity_labels]
            )
            pred_spans_set = set(
                [x for x, y in predicted_spans.items() if y not in non_entity_labels]
            )
    ...
    def get_metric(self, reset: bool = False) -> float:
        ...
        for tag in all_tags:
            precision, recall, f1_measure, f2_measure = self.compute_prf_metrics(
                true_positives=self._TP[tag],
            ...
            all_metrics["F2@{}".format(tag)] = f2_measure

        # Compute the precision, recall and f1 for all spans jointly.
        precision, recall, f1_measure, f2_measure = self.compute_prf_metrics(
            true_positives=sum(self._TP.values()),
        ...
        all_metrics["micro@F2"] = f2_measure
        ...
        if self._num_predicted_mentions == 0:
            ...
            entity_precision = self._num_recalled_mentions / float(
                self._num_predicted_mentions
            )
    ...
    def compute_prf_metrics(
        ...
        f1_measure = compute_fbeta(precision, recall, beta=1.0)
        f2_measure = compute_fbeta(precision, recall, beta=2.0)
        return precision, recall, f1_measure, f2_measure
